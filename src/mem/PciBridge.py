...
class PciBridge(ClockedObject):
    ...
    is_switch = Param.Int(
        0,
        "Whether this is a switch and virtual p2p bridge needed for upstream port",
    )
    ...
    SubordinateBusNumber = Param.UInt8(
        0x00, "LArgest bus number downstream of bridge"
    )
    host = Param.PciHost(Parent.any, "PCI host")
    pci_bus = Param.Int("PCI bus")
    pci_upstream_dev = Param.Int(
        0, "PCI device number of upstream switch port"
    )
    ...
    DeviceId_upstream = Param.UInt16(
        0x8232, "P2P Bridge's device Id"
    )  # For upstream switch port
    ...
    MemoryLimit = Param.UInt16(
        0x0000, "Memory limit for devices downstream of bridge"
    )
    MemoryBase = Param.UInt16(
        0x0000, "Memory Base foe devices downstream of Bridge"
    )
    ...
    PrefetchableMemoryLimitUpper = Param.UInt32(
        0x00000000, "Upper 32 bits of prefetchable memory limit"
    )
    PrefetchableMemoryBaseUpper = Param.UInt32(
        0x00000000, "Upper 32 bits of prefetchable memory base"
    )
    ...
    CapabilityPointer = Param.UInt8(
        0x00, "Pointer to extended capabilities if any"
    )
    ExpansionROMBaseAddress = Param.UInt32(
        0, " Base address for ROM if it exists"
    )
    InterruptPin = Param.UInt8(0, "PCI interrupt pin")
    InterruptLine = Param.UInt8(
        0, "Interrupt line used in inteerupt controller"
    )
    BridgeControl = Param.UInt16(0, "Bridge control register")
    SecondaryStatus = Param.UInt16(
        0, "Bridge's secondary interface status register"
    )
    ...
    PXCAPCapabilities_downstream = Param.UInt16(
        0x0061, "PCIe Capabilities for downstream switch port"
    )
    PXCAPCapabilities_upstream = Param.UInt16(
        0x0051, "PCIe Capabilities for upstream switch port"
    )
